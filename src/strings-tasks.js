...
function sumOfCodes(str) {
  ...
  while (count < str.length) {
    sum += arrayOfString[count].charCodeAt();
    count += 1;
  }
  return sum;
}
...
function formatTime(minutes, seconds) {
  return `${minutes.toLocaleString().padStart(2, '0')}:${seconds
    .toLocaleString()
    .padStart(2, '0')}`;
}
...
function countVowels(str) {
  ...
  str.split('').forEach((item) => {
    if (
      vowels.find(
        (vowel) => vowel.toLocaleLowerCase() === item.toLocaleLowerCase()
      )
    ) {
      sum += 1;
    }
...
function isPalindrome(str) {
  const lettersArray = str.split('').filter((item) => {
    if (item === ' ' || item === ',' || item === '!' || item === '?') {
      return '';
    }
    return item;
  });
  const letters = lettersArray.join('').toLocaleLowerCase();
  const reverseLetters = lettersArray.reverse().join('').toLocaleLowerCase();
...
function findLongestWord(sentence) {
  ...
  sentence.split(' ').forEach((item) => {
    if (item.length > largeCount) {
      largeCount = item.length;
      largeWord = item;
    }
  });
  return largeWord;
...
function extractNameFromTemplate(value) {
  ...
  return `${array[1]} ${array[2].replace('!', '')}`;
}
...
function encodeToRot13(str) {
  const codeArr = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm'.split(
    ''
  );
  const alfavit = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const codeStr = [];
  str.split('').forEach((item) => {
    const codingLetter = codeArr[alfavit.indexOf(item)];
    if (codingLetter) {
      codeStr.push(codingLetter);
    } else {
      codeStr.push(item);
    }
  });
